...
/*:
 En este ejemplo, `string` (cadena) es del tipo `String` porque se crea a partir de un literal. Y `anotherString` (otra cadena) también es del tipo `String` (cadena) porque se crea a partir de un tipo `String`.

 - callout(Experimento): Cambia `"42"` por `42`. ¿Qué tipo tendrá `anotherString` (otra cadena) ahora?\
(Pista: Para comprobar el tipo, mantén presionada la tecla `Opción` y haz clic en la constante `anotherString` [otra cadena]).
