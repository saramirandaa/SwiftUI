/*:
## Índices
El siguiente es un arreglo de dispositivos:
 */
...
/*:
 ...
 Ese número se conoce como el _índice_ del elemento en el arreglo y representa su posición en la línea. En este caso, “iPod” está en el índice 2 del arreglo `devices` (dispositivos). Como este arreglo está definido con `let`, nunca cambian los elementos ni su orden. Esto significa que no importa cuántas veces imprimas el arreglo, `"iPod"` siempre estará en el índice 2.

 - callout(Nota): Verás que, en el ámbito informático, el verbo “indizar” (que viene de índice) también se usa como “indexar”.
...
//:  - callout(Experimento): Declara una constante `favoriteDevice` (dispositivo preferido) y establece su valor en “iPod” usando un índice en el arreglo `devices` (dispositivos).
// Declara la constante favoriteDevice a continuación
...
/*:
 ...
 - callout(Experimento ): Intenta obtener el elemento en el índice `4` de la lista. Abre la consola para obtener más información sobre el error.
 */

/*:
A continuación, averiguarás cuántos valores contiene un arreglo.\
[Anterior](@previous)  |  página 3 de 18  |  [Siguiente: Recuento](@next)
 */