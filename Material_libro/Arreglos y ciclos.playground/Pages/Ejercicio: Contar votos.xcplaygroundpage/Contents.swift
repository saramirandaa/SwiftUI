...
## Ejercicio: Contar votos
 ...
 ¡Es mucha información! Pero no te preocupes por la longitud de los arreglos. Ya sea que estés planeando repetir el proceso con dos elementos o con dos mil, deberás escribir el ciclo de la misma forma.
 */
...
//:  - callout(Ejercicio): En este ejercicio, debes crear dos variables, una para contar los votos `yes` (sí) y la otra para contar los votos `no`. Cada una debe comenzar con el valor cero.
//:  - callout(Ejercicio): Crea un ciclo “for...in” que itere a lo largo de un arreglo de votos y compruebe el valor de cada voto. Si el voto tiene valor `true` (verdadero), el ciclo debe agregar un voto a la variable `yes` (sí). Si tiene valor `false` (falso), debe agregar un voto a la variable `no`.

//:  - callout(Ejercicio): Cuando termine de ejecutarse el ciclo, escribe una instrucción condicional “if” que compare los dos valores e imprima distintos mensajes según el resultado positivo o negativo de la votación.

/*:
 - callout(Ejercicio): Prueba el código llamando al ciclo “for...in” en cada arreglo de votos.\
...
### Extensión:
 ...
 - callout(Ejercicio): Escribe una función que tenga dos argumentos: una cadena que describa el problema por el que se está votando y un arreglo con los votos `Bool` (booleanos) sobre el problema. Mueve el ciclo “for...in” y la instrucción condicional “if” hacia *adentro* de la función, de modo que se impriman los resultados cuando llames a la función con argumentos de un problema en particular. Deberías poder llamar a la función de esta manera:
 */
 printResults(forIssue: "¿Debemos cambiar de mascota?", withVotes:shouldMascotChangeVotes)
/*:
...
/*:
[Anterior](@previous)  |  página 15 de 18  |  [Siguiente: Ejercicio: Metas](@next)
 */