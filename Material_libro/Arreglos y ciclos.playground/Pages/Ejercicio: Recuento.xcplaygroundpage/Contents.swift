...
Los únicos índices de un arreglo que es seguro usar son aquellos con un número inferior al resultado de `count`.
 ...
 - callout(Ejercicio): Siguiendo lo que aprendiste sobre tomar decisiones, escribe una instrucción condicional “if” que únicamente acceda al arreglo si el valor de `index` (índice) es inferior al recuento del arreglo. Elimina el comentario y actualiza el código que se encuentra debajo. Luego, actualiza la constante `index` con distintos números para ver el resultado.
 */
let index = 3
...
/*:
[Anterior](@previous)  |  página 13 de 18  |  [Siguiente: Ejercicio: Anfitrión del karaoke](@next)
 */