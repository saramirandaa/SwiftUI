...
/*:
 La instrucción “switch” no tiene un elemento “case” para cada valor posible de la enum. Por el contrario, hay una palabra clave `default` (opción predeterminada) que se usará si no se encuentran otras coincidencias. Esto es similar a la cláusula `else` final cuando se usa una instrucción condicional “if”.

 - callout(Experimento): Cambia el valor de `quality` (calidad) para probar cuándo se usa el elemento “case” `default` y cuándo se usan elementos “case” específicos.\
\
