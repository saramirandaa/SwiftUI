/*:
## Nomenclatura
En programación, los nombres pueden ser muy descriptivos y útiles para identificar cada cosa, como los nombres que se asignan a objetos de la vida real. En el código, después de ponerle un nombre a un valor, se puede usar ese nombre en todos los casos en los que utilizarás el valor. Esta es una forma de *abstracción* que facilita la lectura y la comprensión del código.

Además, los nombres ayudan a evitar errores. Por ejemplo, era fácil cometer un error en el experimento del espectáculo de mascotas, ya que había que cambiar un número en varias líneas de código para cada actualización. Sin embargo, si hubieras definido un nombre para cada valor (por ejemplo, la cantidad de tortugas), solo habrías tenido que cambiar la definición del nombre una vez, y luego se usaría la definición actualizada en todos los casos en los que apareciera el nombre en el código.

En Swift, puedes elegir un nombre y asociarlo con un valor mediante la definición de una _constante_. Analizarás las constantes en detalle más adelante en el curso. Por el momento, es suficiente con saber que las constantes son fundamentales para asignar nombres a los valores en Swift.
...
//: Una vez que hayas definido una constante y le hayas asignado un valor, puedes usar la constante en todos los casos para reemplazar el valor, incluidos los cálculos matemáticos que definen el valor de otra constante:
...
Incluso puedes dejar que el playground realice los cálculos de suma o resta a la derecha del signo igual. Por ejemplo, si dos personas más llevan un cerdo, en lugar de cambiar la cantidad de 6 a 8, podrías escribir:

`let numberOfDogs = 6 + 2`

 - callout(Experimento): En el transcurso de la semana, obtienes aún más información sobre las mascotas de las personas.\
