...
## Crear una instancia
 ...
 Hasta el momento, creaste casi todas las instancias escribiendo directamente un valor literal en el código. La única excepción fue en el playground Tipos. Allí usaste `Date()` para crear un valor con la fecha actual:
 */
import Foundation
...
Esto es un ejemplo de un _inicializador_. Los inicializadores se utilizan para crear una nueva instancia de un determinado tipo. Muy pocos tipos, como `String` (cadena), `Bool` (booleano) e `Int` (entero), se pueden crear usando literales, pero cada tipo tiene al menos un inicializador.
...
/*:
 El inicializador te da un valor de tipo `Date` (fecha), es decir, un número que representa los segundos de la hora actual.
