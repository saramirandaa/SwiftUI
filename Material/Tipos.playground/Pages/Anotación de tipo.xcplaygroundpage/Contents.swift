...
## Anotación de tipo
 ...
 - callout(Experimento): Elimina el comentario de la siguiente línea de código y observa el error. Cuando hayas terminado, vuelve a convertirla en comentario.
 */
//let mysteryConstant
/*:
 ...
 También es posible que no siempre quieras que Swift use la inferencia de tipo, ya que podría no devolver el tipo que quieres, como en el cálculo con los tipos `Double` (doble) e `Int` (entero) que intentaste anteriormente.
...
result * 0.5
/*:
- `annotatedDouble` (doble anotado) es un tipo `Double` (doble), aunque no haya punto decimal, por la anotación de tipo.
- `inferredDouble` (doble inferido) es un tipo `Double` (doble) porque se escribe con un punto decimal.
- `result` (resultado) es un tipo `Double` (doble) porque el resultado de `Double * Double` es un tipo `Double`.
