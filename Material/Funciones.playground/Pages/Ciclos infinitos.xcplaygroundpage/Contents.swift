...
## Ciclos infinitos
 ...
 A veces, los programadores cometen el error de llamar a una función desde la función _misma_. Cuando se llama a la función, esa función se llama a sí misma, y esa función se llama a sí misma, y esa función se llama a sí misma…
...
/*:
 - callout(Experimento): Crea un ciclo infinito en el código anterior editando la función `verseOne` (primera estrofa) para que llame a `verseOne()` (primera estrofa) después de llamar a `merrilyDream()` (remar rápido el barquito). Observa la consola y la barra lateral de resultados. Elimina la línea para detener el ciclo. Es posible que el playground demore en recuperarse. Los ciclos infinitos requieren bastante trabajo.
